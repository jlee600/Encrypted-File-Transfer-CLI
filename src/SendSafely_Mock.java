...
/**
 * SendSafely Java CLI Application
 */
public class SendSafely_Mock {

    private static SendSafely client;      // SendSafely API client
    private static Package pkgInfo;         // Current package information
    private static Stack<Command> undoStack = new Stack<>();  // Undo feature stack
    private static Scanner scanner = new Scanner(System.in);

    public static void main(String[] args) {
        System.out.println("Welcome to the SendSafely CLI Application");

        try {
            authenticate();
            pkgInfo = client.createPackage();
            System.out.println("Package created successfully with ID: " + pkgInfo.getPackageId());
            interactiveLoop();
        } catch (Exception e) {
    ...
    private static void authenticate() throws Exception {
        ...
        System.out.println("Hello " + userInformation.getFirstName() + "!");
        System.out.println("Successfully connected to SendSafely as user " + userInformation.getEmail());
    }
    ...
    private static void handleUpload() {
        ...
        try {
            ...
            // Create an instance of the progress callback
            Progress progressCallback = new Progress(file.getName());

            // First, define a file manager using the DefaultFileManager
            FileManager fileManager = new DefaultFileManager(new java.io.File(filePath));

            // Now call encryptAndUploadFile by passing the fileManager and the file
            ...
            // After the upload completes, the progress callback should have printed progress updates.
            System.out.println("Upload complete for file: " + file.getName());

            // Store undo action for file upload
    ...
    private static void handleAddRecipient() {
        ...
        try {
            client.addRecipient(pkgInfo.getPackageId(), email);
            System.out.println("Recipient added: " + email);

            // Store undo action for adding recipient
            undoStack.push(new AddRecipientCommand(pkgInfo.getPackageId(), email, client));
        } catch (Exception e) {
...
class AddRecipientCommand implements Command {
    ...
    public AddRecipientCommand(String packageId, String recipientEmail, SendSafely client) {
        this.packageId = packageId;
    ...
    public void undo() throws Exception {
        try {
            client.removeRecipient(packageId, recipientEmail);
            System.out.println("Recipient " + recipientEmail + " removed from package.");
        } catch (Exception e) {
...
class Progress implements ProgressInterface {
    ...
    /**
     * Called to update progress (e.g., as a percentage).
     */
    ...
    /**
     * Overloaded method to provide additional context.
     */
    @Override
    ...
    /**
     * Called when the file ID is obtained.
     */
    @Override
